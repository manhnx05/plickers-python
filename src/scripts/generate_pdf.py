...
def build_pdf():
    ...
    for i, filename in enumerate(files):
        ...
        if i % CARDS_PER_PAGE == 0:
            ...
            c.drawCentredString(PAGE_W / 2, PAGE_H - 0.9 * cm,
                                "📋 THẺ PLICKERS — CẦM ĐÚNG HƯỚNG ĐÁP ÁN LÊN TRÊN ĐẦU")
            c.setFont('Helvetica', 9)
            c.setFillColor(colors.grey)
            c.drawCentredString(PAGE_W / 2, PAGE_H - 1.4 * cm,
                                "Hướng nào lên đầu = Đáp án đó.  A=Trên  B=Phải  C=Dưới  D=Trái")
