...
def _draw_hud(frame, scanned_cards: dict, current_time: float) -> None:
    ...
    cv2.putText(frame, "--- DIEM DANH ---", (20, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
...
def main() -> None:
    ...
    while True:
        ...
        for card_id, cnt in found_cards:
            ...
            cv2.putText(
                frame, f"ID: {student_no}", (x, max(y - 10, 20)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 200, 0), 2
            )
        ...
        cv2.putText(
            frame, f"FPS: {int(fps)}", (frame.shape[1] - 120, 35), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2
        )
